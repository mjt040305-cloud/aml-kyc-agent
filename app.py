...
if officer["role"] in SECOND_SIGNER_ROLES:
    ...
    # act on: unsigned, and either open (no assigned_officer_id) or
    # specifically routed to them. A case with no such transactions after
    # filtering is not shown at all - it belongs to someone else's queue.
    visible_queue = []
    for c in cosign_queue:
        actionable = {
            tid: e for tid, e in c["cosign"].items()
            if not e.get("second_officer_id")
            and (not e.get("assigned_officer_id") or e["assigned_officer_id"] == officer["officer_id"])
        }
        if actionable:
            visible_queue.append((c, actionable))
    ...
    if st.session_state.pipeline_status == "awaiting_review":
        ...
        for txn in st.session_state.pending_transactions:
            ...
            with st.container(border=True):
                ...
                assigned_officer_id, assigned_officer_name = None, None
                if txn["risk_bucket"] == "High":
                    ...
                    eligible = [
                        o for o in auth_db.list_officers_by_roles(list(SECOND_SIGNER_ROLES))
                        if o["officer_id"] != officer["officer_id"]
                    ]
                    route_options = ["(open to any qualifying officer)"] + [f"{o['full_name']} ({o['role']})" for o in eligible]
                    route_choice = st.selectbox(
                        "Route co-signature request to", route_options,
                        key=f"route_{txn['transaction_id']}"
                    )
                    if route_choice != route_options[0]:
                        chosen = eligible[route_options.index(route_choice) - 1]
                        assigned_officer_id, assigned_officer_name = chosen["officer_id"], chosen["full_name"]
                ...
                decisions[txn["transaction_id"]] = {
                    "status": status, "reviewer": reviewer, "notes": notes,
                    "assigned_officer_id": assigned_officer_id, "assigned_officer_name": assigned_officer_name,
                }
            ...
            if needs_cosign:
                cosign_requirements = {
                    tid: {
                        ...
                        "assigned_officer_id": decisions[tid].get("assigned_officer_id"),
                        "assigned_officer_name": decisions[tid].get("assigned_officer_name"),
                    }
                    for tid in needs_cosign
                }
                case_store.submit_for_cosign(
                    st.session_state.case_id, officer["officer_id"], officer["full_name"], officer["role"],
                    cosign_requirements
                )
                st.warning(
    ...
    if st.session_state.pipeline_status == "complete":
        ...
        if st.session_state.case_id:
            ...
            if case_status and case_status["status"] == "pending_cosign":
                ...
                cosign = case_status.get("cosign") or {}
                fully_signed = {
